    ...
    # second level tags
        for child in element:
            ...
            for centry in child_attributes:
                if centry == 'k':
    # - if second level tag "k" value contains problematic characters, it should be ignored
                    if problemchars.search(child_attributes['k']):
                        break
    # - if second level tag "k" value starts with "addr:", it should be added to a dictionary "address"
                    elif child_attributes['k'] == 'addr:housenumber':
                        if address == 0:
                            node['address'] = {}
                            address = 1
                        node['address']['housenumber'] = child_attributes['v']
                    elif child_attributes['k'] == 'addr:street':
                        if address == 0:
                            node['address'] = {}
                            address = 1
                        node['address']['street'] = child_attributes['v']
    # - if second level tag "k" value does not start with "addr:", but contains ":", you can process it
  # same as any other tag.
                    elif ':' in child_attributes['k']:
                        node[child_attributes['k']] = child_attributes['v']
                    else:
                        node[child_attributes['k']] = child_attributes['v']
                elif centry == 'ref':
                    if refs == 0:
